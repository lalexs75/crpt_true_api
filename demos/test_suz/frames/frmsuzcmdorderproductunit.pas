...
unit frmSUZCmdOrderProductUnit;
...
type
  ...
  { TfrmSUZCmdOrderProductFrame }

  TfrmSUZCmdOrderProductFrame = class(TfrmSUZCmdAbstractFrame)
    Button1: TButton;
    edtOrderID: TEdit;
    Label2: TLabel;
    Memo1: TMemo;
...
procedure TfrmSUZCmdOrderProductFrame.Button1Click(Sender: TObject);
var
  P1: TJSONData;
begin
...
function TfrmSUZCmdOrderProductFrame.FrameName: string;
begin
  Result:='Атрибуты товара';
end;

procedure TfrmSUZCmdOrderProductFrame.LoadParams(AIni: TIniFile);
begin
  inherited LoadParams(AIni);
  edtOrderID.Text:=AIni.ReadString(ClassName, 'edtOrderID_Text', '');
end;

procedure TfrmSUZCmdOrderProductFrame.SaveParams(AIni: TIniFile);
begin
