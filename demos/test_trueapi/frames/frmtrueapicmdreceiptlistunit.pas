...
unit frmTrueAPICmdReceiptListUnit;
...
type
  ...
  { TfrmTrueAPICmdReceiptListFrame }

  TfrmTrueAPICmdReceiptListFrame = class(TfrmTrueAPICmdAbstractFrame)
    Button1: TButton;
    Memo1: TMemo;
...
procedure TfrmTrueAPICmdReceiptListFrame.Button1Click(Sender: TObject);
var
  R: TJSONData;
begin
  R:=CRPTTrueAPI.ReceiptList(SelectedGroup);
  if Assigned(R) then
  begin
...
function TfrmTrueAPICmdReceiptListFrame.FrameName: string;
begin
  Result:='Список чеков';
end;

procedure TfrmTrueAPICmdReceiptListFrame.LoadParams(AIni: TIniFile);
begin
  inherited LoadParams(AIni);
end;

procedure TfrmTrueAPICmdReceiptListFrame.SaveParams(AIni: TIniFile);
begin
